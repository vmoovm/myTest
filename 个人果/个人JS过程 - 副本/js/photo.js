...
$(document).ready(function() {
	...
	//分配身份//www.wetx.net/33cc/d_d d.gif
	...
	function moveElement(){test.html(test.html()+'moveElement可移动目标 --<br/>');
		...
		conf.m={
			l_n:p_this.find("[status='"+conf.status.l_n+"']"),	//左隐藏 none的简写
			...
			r_n:p_this.find("[status='"+conf.status.r_n+"']")	//右 none的简写
		}
		//test.html(test.html()+conf.m.cur.height()+'插队 --<br/>');
	}
 	moveElement();
	...
	function updateSrc(){test.html(test.html()+'updateSrc更新可移动图片地址 --<br/>');
		conf.s.l_n.src=conf.m.l_n==null?'':conf.m.l_n.find('img').attr('src');
		conf.s.l_s.src=conf.m.l_s==null?'':conf.m.l_s.find('img').attr('src');
		conf.s.l_b.src=conf.m.l_b==null?'':conf.m.l_s.find('img').attr('src');
		conf.s.cur.src=conf.m.cur==null?'':conf.m.cur.find('img').attr('src');
		conf.s.r_b.src=conf.m.r_b==null?'':conf.m.r_b.find('img').attr('src');
		conf.s.r_s.src=conf.m.r_s==null?'':conf.m.r_s.find('img').attr('src');
		conf.s.r_n.src=conf.m.r_n==null?'':conf.m.r_n.find('img').attr('src');
	...
	function getSize(){test.html(test.html()+'getSize获取图片尺寸 --<br/>');
		...
		//原始宽
		conf.w.l_n=objstr.test(conf.s.l_n.src)?0:conf.s.l_n.width;
		conf.w.l_s=objstr.test(conf.s.l_s.src)?0:conf.s.l_s.width;
		conf.w.l_b=objstr.test(conf.s.l_b.src)?0:conf.s.l_b.width;
		conf.w.cur=objstr.test(conf.s.cur.src)?0:conf.s.cur.width;
		conf.w.r_b=objstr.test(conf.s.r_b.src)?0:conf.s.r_b.width;
		conf.w.r_s=objstr.test(conf.s.r_s.src)?0:conf.s.r_s.width;
		conf.w.r_n=objstr.test(conf.s.r_n.src)?0:conf.s.r_n.width;
		//原始高
		conf.h.l_n=objstr.test(conf.s.l_n.src)?0:conf.s.l_n.height;
		conf.h.l_s=objstr.test(conf.s.l_s.src)?0:conf.s.l_s.height;
		conf.h.l_b=objstr.test(conf.s.l_b.src)?0:conf.s.l_b.height;
		conf.h.cur=objstr.test(conf.s.cur.src)?0:conf.s.cur.height;
		conf.h.r_b=objstr.test(conf.s.r_b.src)?0:conf.s.r_b.height;
		conf.h.r_s=objstr.test(conf.s.r_s.src)?0:conf.s.r_s.height;
		conf.h.r_n=objstr.test(conf.s.r_n.src)?0:conf.s.r_n.height;
	}
	...
	function setSite(){test.html(test.html()+'setSite计算位置 --<br/>');
		...
		if(conf.m.l_b!=null) conf.l.l_b=conf.j.bw+conf.j.bw*0.5;
